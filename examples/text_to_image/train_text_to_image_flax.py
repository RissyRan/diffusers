...
def main(start_time_sec):
  ...
  if jax.process_index() == 0:
    ...
    if args.push_to_hub:
      repo_id = create_repo(repo_id=args.hub_model_id or
  ...
  state = train_state.TrainState.create(apply_fn=unet.__call__,
                                        params=unet_params,
                                        tx=optimizer)
  ...
  def train_step(state, text_encoder_params, vae_params, batch, train_rng):
    ...
    def compute_loss(params):
      ...
      vae_outputs = vae.apply({"params": vae_params},
                              batch["pixel_values"],
                              deterministic=True,
                              method=vae.encode)
      ...
      noisy_latents = noise_scheduler.add_noise(noise_scheduler_state, latents,
                                                noise, timesteps)
      ...
      if noise_scheduler.config.prediction_type == "epsilon":
        ...
        target = noise_scheduler.get_velocity(noise_scheduler_state, latents,
                                              noise, timesteps)
      else:
        raise ValueError(
  ...
  for epoch in epochs:
    ...
    for batch in train_dataloader:
      batch = shard(batch)
      state, train_metric, train_rngs = p_train_step(state, text_encoder_params,
                                                     vae_params, batch,
                                                     train_rngs)
  ...
  if jax.process_index() == 0:
    scheduler = FlaxPNDMScheduler(beta_start=0.00085,
                                  beta_end=0.012,
                                  beta_schedule="scaled_linear",
                                  skip_prk_steps=True)
